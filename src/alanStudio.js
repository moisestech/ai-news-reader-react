...
intent('Give me the news from $(source* (.*))', (p) => {
  ...
  api.request(NEWS_API_URL, (error, response, body) => {
    ...
    if (!articles.length) {
      p.play(
        'Sorry, please try searching for news from a different source',
      );
      return;
    }
...
intent("what's up with $(term* (.*))", (p) => {
  ...
  api.request(NEWS_API_URL, (error, response, body) => {
    ...
    p.play({ command: 'newHeadlines', articles });
    p.play(
      `Here are the (latest|recent) articles on ${p.term.value}.`,
    );
...
intent(
  ...
  (p) => {
    ...
    api.request(NEWS_API_URL, (error, response, body) => {
      ...
      if (!articles.length) {
        p.play(
          'Sorry, please try searching for a different category.',
        );
        return;
      }
      ...
      if (p.C.value) {
        p.play(
          `Here are the (latest|recent) articles on ${p.C.value}.`,
        );
      } else {
